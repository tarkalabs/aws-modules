...
EOF
  ...
  settings            = [
    ...
    },
    {
      ...
      value   = dependency.eks_cluster.outputs.managed_node_groups.platform-devops-ng.node_group_autoscaling_group_names.0
    },
    {
      name    = "autoscalingGroups[0].maxSize"
      value   = local.tgvars.platform_devops_ng_auto_scaling_max_size
    },
    {
      name    = "autoscalingGroups[0].minSize"
      value   = local.tgvars.platform_devops_ng_auto_scaling_min_size
    }
  ]
}
